...
public partial class MainWindow : Window
{
    ...
    public MainWindow()
    {
        ...
        Title = $"Uninstall {ProductName}"; HeaderSubtitle.Text = "Uninstalling version 1.0.0"; TitleText.Text = "Uninstall PC Pressure Monitor?";
        DescriptionText.Text = "The monitoring service and application will be removed. Existing snapshot history in ProgramData will remain on this computer.";
        ActionButton.Content = "Uninstall"; ActionButton.Background = System.Windows.Media.Brushes.IndianRed;
    }
    ...
    private async void ActionClick(object sender, RoutedEventArgs e)
    {
        ...
        try
        {
            ...
            Progress.Visibility = Visibility.Collapsed; StatusText.Text = _uninstall ? "Application removed." : "Installation complete.";
            ActionButton.Content = _uninstall ? "Close" : "Launch"; ActionButton.IsEnabled = true; ActionButton.Click -= ActionClick; ActionButton.Click += FinishClick; CancelButton.Visibility = Visibility.Collapsed;
        }
        catch (Exception ex)
        {
            SetBusy(false); StatusText.Text = "Error: " + ex.Message; MessageBox.Show(ex.ToString(), "Setup error", MessageBoxButton.OK, MessageBoxImage.Error);
        }
    }

    private void Install()
    {
        UpdateStatus("Stopping the previous version…");
        ...
        var currentSetup = Environment.ProcessPath ?? throw new InvalidOperationException("Unable to determine the setup path.");
    ...
    {
        UpdateStatus("Stopping the Windows service…"); RunSc(false, "stop", ServiceName); Thread.Sleep(1200); RunSc(false, "delete", ServiceName);
    ...
    {
        using var source = Assembly.GetExecutingAssembly().GetManifestResourceStream(resource) ?? throw new InvalidOperationException($"The setup package is damaged: {resource}");
    ...
    private static void RunSc(bool required, params string[] args)
    {
        ...
        using var process = Process.Start(info) ?? throw new InvalidOperationException("Unable to start Service Control Manager."); process.WaitForExit(30000);
    ...
    private void SetBusy(bool value) { ActionButton.IsEnabled = !value; CancelButton.IsEnabled = !value; Progress.Visibility = value ? Visibility.Visible : Visibility.Collapsed; StatusText.Text = value ? "Preparing…" : ""; }
